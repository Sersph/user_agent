...
var (
	//iphone  = `mozilla/5.0 (iphone; cpu iphone os 14_7 like mac os x) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.1.2 Mobile/15E148 Safari/604.1`
	android = `mozilla/5.0 (linux; android 11; m2012k10c build/rp1a.200720.011; wv) applewebkit/537.36 (khtml, like gecko) version/4.0 chrome/87.0.4280.141 mobile safari/537.36`
	androie = `mozilla/5.0 (linux; android 10; v1950a build/qp1a.190711.020; wv) applewebkit/537.36 (khtml, like gecko) version/4.0 chrome/83.0.4103.106 mobile safari/537.36;`
)

func TestModel(t *testing.T) {

	a := "mozilla/5.0 (linux; android 11; penm00 build/rkq1.201105.002; wv) applewebkit/537.36 (khtml, like gecko) version/4.0 chrome/83.0.4103.106 mobile safari/537.36;"
	...
	uaM := ua.Platform() + ua.OS() + ua.Model() + ua.Mozilla() + name + version
	fmt.Println("mm : ", uaM)
}

func TestMode(t *testing.T) {
	//fmt.Println("androif :", androif)

	gei := `mozilla/5.0\s*\([^\(\)]*?(android[^\(\)]*?|iphone);([^\(\)]*?)\)`

	r := regexp.MustCompile(gei)
	matchs := r.FindStringSubmatch(androie)
